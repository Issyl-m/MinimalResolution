...
from sage.rings.finite_rings.finite_field_constructor import GF # WARNING (BAD PERFORMANCE)
from sage.matrix.constructor import matrix as Matrix # WARNING (BAD PERFORMANCE)
from sage.algebras.steenrod.steenrod_algebra import SteenrodAlgebra # WARNING (BAD PERFORMANCE)
...
def print_header(str_txt, char_delim, bool_print_lateral):
    ...
    print(char_delim_lateral + ' '*padding_length + str_txt + ' '*(padding_length + padding_tweak) + char_delim_lateral)
    print(char_delim*number_of_characters)
...
class Morphism: 
    def __init__(self, fixed_prime, list_dom_basis, list_cod_basis, list_list_images, tuple_dom=(-1,-1), tuple_cod=(-1,-1)): ## TODO: diff. coord.
    ...
    def eval_linear_comb(self, list_list_linear_comb):
        ...
        return self.matrix * Matrix(GF(self.fixed_prime), element_as_vector, sparse=True)
    ...
    def OLDgetListListMatrix(self, list_dom_basis, list_cod_basis, list_list_images): ## TODO: remove
    ...
    def getListListMatrix(self, list_dom_basis, list_cod_basis, list_list_images):
        ...
        for i in range(0, dim_dom):
            for j in range(0, dim_cod):
                cod_basis_trailing_support = list_cod_basis[j].cohomology_operation.trailing_support()
                cod_basis_leading_coeff = list_cod_basis[j].cohomology_operation.leading_coefficient()

                for k in range(0, len(list_list_images[i])):
                    image_cohomology_operation = list_list_images[i][k].cohomology_operation
                    dict_image_cohomology_operation_support = image_cohomology_operation.support()._mapping
                    list_keys_dict_image_cohomology_operation_support = list(dict_image_cohomology_operation_support.keys())

                    for monomial_index in range(0, len(list_keys_dict_image_cohomology_operation_support)):
                        monomial_support = list_keys_dict_image_cohomology_operation_support[monomial_index]
                        monomial_coefficient = dict_image_cohomology_operation_support[monomial_support]
                        
                        if list_list_images[i][k].generator == list_cod_basis[j].generator:
                            bool_monomial_patch = False ## TODO: warning...
                            if monomial_support in [tuple([]), (0,)] and cod_basis_trailing_support in [tuple([]), (0,)]:
                                bool_monomial_patch = True
                            if monomial_support == cod_basis_trailing_support or bool_monomial_patch:
                                list_list_matrix[j][i] += monomial_coefficient/cod_basis_leading_coeff # 1, ..., p-1
        
        #DBG(self.OLDgetListListMatrix(list_dom_basis, list_cod_basis, list_list_images) == list_list_matrix)
...
class MinimalResolution:
    def __init__(self, str_name, fixed_prime, number_of_modules, number_of_relative_degrees):
        self.str_name = str_name
    ...
    def createModule(self, fp_module):
        ...
        for ev_module_generator in self.list_module_to_resolve_ev_gen:
            if ev_module_generator.generator.bool_is_free:
                ...
                lift_ev_module_free_generator = Element(self.A_unit, lifted_generator_info)
            ...
            if len(self.list_list_found_generators) > resolution_module_subindex:
                for found_generators in self.list_list_found_generators[resolution_module_subindex]:
                    if found_generators.deg <= abs_deg:
                        for found_element in self.getSteenrodAlgebraBasis(abs_deg - found_generators.deg):  
    ...
    def sum(self, list_elements):
        ...
        trivial_element = Element(self.A_unit, AlgebraGenerator(-2, 0, 0)) ## fix/change
    ...
    def construct(self):
        for resolution_module_subindex in range(0, MAX_NUMBER_OF_MODULES):
            ...
            for module_relative_deg in range(0, MAX_NUMBER_OF_RELATIVE_DEGREES - resolution_module_subindex): # MAX_NUMBER_OF_RELATIVE_DEGREES (t-s)
                self.diff(resolution_module_subindex, module_relative_deg)

            print(f"[*] Computing minimal resolution until relative degree: {MAX_NUMBER_OF_RELATIVE_DEGREES - resolution_module_subindex - 1}.")
    ...
    def create_free_graded_morphism(self, fp_map, callback_get_domain, callback_get_codomain, dom_module_index, cod_module_index): ## TODO: BUG: varias relaciones
        ...
        for i in range(0, MAX_NUMBER_OF_RELATIVE_DEGREES - dom_rel_deg): ## TODO: optimize
    ...
    def multiprocess_cochain_lift(self, external_resolution, callback_generator_filter, callback_max_module_index):
        with multiprocessing.Manager() as manager:
            ...
            for generator_i in range(0, len(rearranged_found_generators)):
                ...
                self.list_lift_processes.append(multiprocessing.Process(target=self.lift_cochain, args=(self, external_resolution, element_generator_to_lift, callback_max_module_index(generator_to_lift), list_list_output, generator_i, manager_lock)))
